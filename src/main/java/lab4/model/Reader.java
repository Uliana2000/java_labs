...
public class Reader {
    ...
    @NotNull(message = "cant be null")
    @NotEmpty(message = "cant be empty")
    @Telephone(message = "must be in correct international format")
    ...
    public static class Builder {
        ...
        public Reader build() throws IllegalStateException {
            Validator validator = Validation.buildDefaultValidatorFactory().getValidator();
            Set<ConstraintViolation<Reader>> constraintViolations = validator.validate(reader);

            //Show errors
            if (constraintViolations.size() > 0) {
                Set<String> exceptionDetails = new HashSet<>();
                for (ConstraintViolation<Reader> violation : constraintViolations) {
